...
def freeze_ViT_blocks(model, nblocks=2):
    count = 0
    for i,child in enumerate(model.backbone.net.children()):
        if i==0:
            continue
        if count <= nblocks: #don't freeze input layer
            for name, param in child.named_parameters():
                if count > nblocks:
                    break
                param.requires_grad = False
                print(f"{name} frozen!")
                count = int(name[:name.find('.')])
...
def freeze_resnet_layers(model, nlayers=2, unfreeze_first_block = False):
    try:
        children = model.backbone.body.children()
...
def parse_opt():
    ...
    parser.add_argument(
        ...
        default=2,
        type=int ,
        help='layers in backbone for freezing'
    )
    parser.add_argument(
        '--unfreeze_first', 
        dest='unfreeze_first', 
        action='store_true',
        help='unfreeze first block of resnet'
    )
    parser.add_argument(
        '--eval_only', 
        dest='eval_only', 
    ...
    parser.add_argument(
        ...
        dest='random_init', 
        action='store_true',
        help='initialize weights randomly'
    )
    parser.add_argument(
        '--val_freq',
        default=10,
...
def main(args):
    ...
    if args['weights'] is None:
        ...
        if args['random_init']:
            ...
            pt = True
        model = build_model(num_classes=NUM_CLASSES, pretrained=pt)
        print(f'Building model from {"torchvision" if pt else "random"} weights...')
    ...
    if args['weights'] is not None:        
        ...
        try:
            ...
            print(e)
            print("Skipping loading missing keys...")
            model.load_state_dict(ckpt_state_dict, strict=False)
        model.to(DEVICE)
        del ckpt_state_dict
...
#         print(f"Model layers 1 - {count} frozen")
    ...
    try:
        torchinfo.summary(
            model, 
            device=DEVICE, 
            input_size=(BATCH_SIZE, args['in_chans'], IMAGE_SIZE, IMAGE_SIZE),
    ...
    for epoch in range(start_epochs, NUM_EPOCHS):
        ...
        if epoch % args['val_freq'] == 0 or epoch == start_epochs:
            stats, val_pred_image = evaluate(
                model, 
                valid_loader, 
                device=DEVICE,
